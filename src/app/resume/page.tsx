  ...
  return (
    <div className={s.container}>
      ...
      <div className={s.actionContainer}>
        <button type="button" className={s.uploadButton}>파일 업로드</button>
        <button type="button" className={s.createButton}>이력서 작성하기</button>
      </div>
        ...
        {[1, 2, 3].map((item) => (
          <div className={s.resumeItem} key={item}>
            ...
            <div className={s.actionLinks}>
              <button className={s.downloadLink}>다운로드</button>
              <button className={s.deleteLink}>삭제하기</button>
            </div>
          </div>
        ))}
      </div>
